...
def play(initial_settings, output_path):
    context = Context()
    ...
    for rounds in tqdm(range(1, context.rounds_to_play)):
        ...
        for player in context.players_list:
            context.cash_record.loc[f'{rounds}', player['name']] = player['cash']
    context.cash_record.to_csv(f'{output_path}\\cash_record_{context.rounds_to_play}.csv')


init_settings = {
    'rounds_to_play': 100004,
    'stake': 10,
    'start_cash': 1000
}
