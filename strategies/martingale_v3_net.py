...
本策略一共分了6个level来买入。
v2版本的，是越跌，仓位越低。所以每个级别有判断仓位的条件，仓位比价格级别要求仓位的大了，就要减仓到目标仓位。但是价格不涨回原理参考价，仓位不上升。
V3版本，跌幅
2021/03/03 zxc update
            ...
            if self.buy_dip_status == 0:
                if (vix < go_safety_vix) and (len(security) == 0):
                    ...
                    if (stock_price - self.reference_price)/self.reference_price < -0.05:
                        ...
                        if (-0.1 < (stock_price - self.reference_price)/self.reference_price < -0.05) and\
                                ((self.positions_value > self.equity *level_2) or (self.positions_value < self.equity *level_2)):
                            amount = round((self.equity * level_2 - self.positions_value)/stock_price)
                            self.enter_stock_order(self.underlying_symbol, stock_price, amount)
                            self.martingale_status = 1
                            self.rebalance_flag = 1
                        elif (-0.15 < (stock_price - self.reference_price)/self.reference_price < -0.1) and\
                                ((self.positions_value > self.equity *level_3) or (self.positions_value < self.equity *level_3)):
                            amount = round((self.equity * level_3 - self.positions_value)/stock_price)
                            self.enter_stock_order(self.underlying_symbol, stock_price, amount)
                            self.martingale_status = 1
                            self.rebalance_flag = 1
                        elif (-0.2 < (stock_price - self.reference_price)/self.reference_price < -0.15) and \
                                ((self.positions_value > self.equity *level_4) or (self.positions_value < self.equity *level_4)):
                            amount = round((self.equity * level_4 - self.positions_value)/stock_price)
                            self.enter_stock_order(self.underlying_symbol, stock_price, amount)
                            self.martingale_status = 1
                            self.rebalance_flag = 1
                        elif (-0.3 < (stock_price - self.reference_price)/self.reference_price < -0.20) and \
                                ((self.positions_value > self.equity *level_5) or (self.positions_value < self.equity *level_5)):
                            amount = round((self.equity * level_5 - self.positions_value)/stock_price)
                            self.enter_stock_order(self.underlying_symbol, stock_price, amount)
                            self.martingale_status = 1
                            self.rebalance_flag = 1
                        elif ((stock_price - self.reference_price)/self.reference_price < -0.30) and \
                                ((self.positions_value > self.equity *level_6) or (self.positions_value < self.equity *level_6)):
